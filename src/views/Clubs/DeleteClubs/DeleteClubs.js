...
import {deleteClubEvent,deletedEvent} from '../../../redux/actions/deleteEventAction';
...
class DeleteClubs extends Component {
    ...
    componentDidMount(){
        ...
        .then(res => {
            ...
            {
                // works
                this.setState({ allEvents: [...this.state.allEvents,res[parseInt(j)]]});
  ...
  dynamicSort(property) {
    ...
    deleteEvent(){
        ...
        this.props.deleteClubEvent(this.state.deleteClubName, this.state.deleteClubId)
    }
    handleClubNameChange(club){
        if(club != null){
            this.setState({clubUrl :club});
            console.log(`Option selected:`, club._url);
            fetch("https://cors-anywhere.herokuapp.com/https://www.mycampusmate.com/api/organizations/" + club._url).then(res => res.json())
            .then(res => {
            for(var i = 0; i<res.events.length; i++)
            {
                console.log(res.events[i].id)
                this.setState({ eventName: [...this.state.eventName,res.events[i]]});
              ...
              <CardGroup>
                <Card className="p-4">
                  <CardBody>
                    ...
                    <Button onClick={this.deleteEvent} color="primary" className="mt-3" active>Delete Event!</Button>
                    </CardBody>
                </Card>
              </CardGroup>
            </Col>
...
export default withRouter(connect(mapStateToProps,{deleteClubEvent,deletedEvent})(DeleteClubs));
